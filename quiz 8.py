...
USER_PANEL_MESSAGE_ID = 1531881552253030503        # !recommendadminpanelでの更新時に随時同期する、実運用中の応募受付パネルのメッセージID
...
class AddDeptModal(discord.ui.Modal, title="新しい部署の追加"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await refresh_user_panel(self.bot_ref)
        try:
...
async def refresh_user_panel(bot: commands.Bot):
    """部署の追加/削除・質問追加・募集ON/OFF切替など、!recommendadminpanel経由でquiz_configが
    更新されるたびに呼び出す。実運用中の応募受付パネル（USER_PANEL_MESSAGE_ID）を
    最新の部署一覧・受付状況で作り直したViewに差し替え、選択肢を常に最新の状態に保つ。"""
    try:
        channel = bot.get_channel(PANEL_AUTO_SEND_CHANNEL_ID)
        if channel is None:
            channel = await bot.fetch_channel(PANEL_AUTO_SEND_CHANNEL_ID)
        if channel is None:
            print("⚠️ [Quiz] 応募受付パネル更新先チャンネルの取得に失敗しました。")
            return

        msg = await channel.fetch_message(USER_PANEL_MESSAGE_ID)
        embed = discord.Embed(
            title="✨ 自己推薦・応募受付",
            description="下のメニューから応募したい部署を選択してください。",
            color=discord.Color.green()
        )
        await msg.edit(embed=embed, view=QuizUserPanelView())
    except discord.NotFound:
        print(f"⚠️ [Quiz] 応募受付パネル(ID:{USER_PANEL_MESSAGE_ID})が見つかりませんでした。メッセージが削除・移動された可能性があります。")
    except Exception as e:
        print(f"❌ [Quiz] 応募受付パネルの自動更新に失敗しました: {e}")
...
def setup_quiz_commands(bot: commands.Bot, *args, **kwargs):
    ...
    async def test_save_cmd(ctx: commands.Context):
        ...
        await refresh_user_panel(ctx.bot)
