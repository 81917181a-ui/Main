...
def _build_ticket_topic_string(data: dict) -> str:
    return "|".join([
        ...
        f"requester:{data.get('requester', '')}",
...
class NewTicketModal(discord.ui.Modal, title="運転整理チケット作成"):
    difficulty = discord.ui.TextInput(label="作成難易度", required=True, max_length=50)
    section = discord.ui.TextInput(label="運転区間", required=True, max_length=100)
    ticket_type = discord.ui.TextInput(label="種別", required=True, max_length=50)

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        await create_new_ticket_channel(interaction, self.difficulty.value, self.section.value, self.ticket_type.value)


async def create_new_ticket_channel(interaction: discord.Interaction, difficulty: str, section: str, ticket_type: str):
    guild = interaction.guild
    user = interaction.user
    ...
    embed = discord.Embed(
        title=f"🎫 チケット #{number}",
        description=f"{user.mention} さんより新規チケットが作成されました。",
        color=discord.Color.green(),
        timestamp=datetime.datetime.now(JST),
    )
    embed.add_field(name="作成難易度", value=difficulty, inline=True)
    embed.add_field(name="運転区間", value=section, inline=True)
    embed.add_field(name="種別", value=ticket_type, inline=True)
    embed.add_field(name="依頼者", value=user.mention, inline=False)
    embed.add_field(name="依頼日", value=requested_date, inline=True)
    embed.set_footer(text="担当者未定 - !assign で担当を設定してください")
...
class NewTicketPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="チケットを開く", style=discord.ButtonStyle.primary, custom_id="new_ticket_panel_open")
